...
# First Three graphs in pp are below
...
dfMedianABVAndIBUByState %>% arrange(dfMedianABVAndIBUByState$MeanIBU)
#mean(dfMedianABVAndIBUByState$MeanIBU%>% filter(!is.na(IBU)))
...
### The state which contains the brewery that produces the beer with the highest alcohol content (ABV) is Oregon.

### The state that ontains the brewery that produces the beer with the highest bitterness rating(IBU) is Colorado.

```{r}
#Prints Max ABV state
